const {ENTER_EVENT,  DROP_EVENT, ERROR_EVENT, SYNC_EVENT, ZOOM_EVENT,MAGNET_EVENT,PLAY_EVENT,QUESTION_EVENT} = require('./events');
...
module.exports = (http) => {
    ...
    io.on('connection', socket => {
        ...
        socket.on(ENTER_EVENT, msg => { //確認用入室イベント
        ...
        socket.on(DROP_EVENT, msg => {　//誰かが座標登録した
            let i=0;
            for(var value of msg.coords) { // オブジェクトの中のプロパティ名を取り出す
            ...
            init = false;
            i=0;
            for(var value of msg.absolute) { // オブジェクトの中のプロパティ名を取り出す。
                absolute[i] = value;
                i++;
              }
        ...
        socket.on(ZOOM_EVENT, msg => {
            ...
            socket.broadcast.emit(ZOOM_EVENT,msg);
            console.log(ZOOM_EVENT,{absolute});
        });
        ...
        socket.on(MAGNET_EVENT, msg =>{
            let i=0;
            for(var value of msg.magnetCoords) { // オブジェクトの中のプロパティ名を取り出す。

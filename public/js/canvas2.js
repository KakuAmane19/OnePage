...
function showFlame(coords){
  ...
  if(isZoomed){//拡大写像
    ...
    scale = (pdfCanvas.width/(absolute.rx-absolute.lx) < pdfCanvas.height/(absolute.ry-absolute.ly))?
                  pdfCanvas.width/(absolute.rx-absolute.lx) : pdfCanvas.height/(absolute.ry-absolute.ly);
  ...
  if(coords.isLocal){
    redFlameContext.clearRect(0, 0, redFlameCanvas.width, redFlameCanvas.height);
    blueFlameContext.clearRect(0, 0, blueFlameCanvas.width, blueFlameCanvas.height);
    blueFlameContext.strokeStyle = '#00f';
    blueFlameContext.strokeRect(flame.lx,
                                flame.ly,
                                flame.rx-flame.lx,
                                flame.ry-flame.ly);
  }else{
    ...
    redFlameContext.strokeRect(flame.lx,
                                flame.ly,
                                flame.rx-flame.lx,
                                flame.ry-flame.ly);
  }
...
(async () => {
  ...
  if(Object.isExtensible(socket)){
    ...
    /*拡大縮小*/
    socket.on(ZOOM_EVENT,msg =>{//誰かがZOOM押したとき
      ...
      let magnetX = magnetCoords[0];
      let magnetY = magnetCoords[1];
      ...
      if(msg.command == "Expansion"){
        ...
        let scale = (pdfCanvas.width/square.sw < pdfCanvas.height/square.sh)?
        ...
        pdfPage.render(renderContext).promise.then(function(){
          ...
          if(  shared.lx <= magnetX
            ...
            && magnetY + magnet.height <= shared.ry ){
              ...
              console.log(magnet.style.left+":"+magnet.style.top,"scale"+scale);
            }else{
        ...
        pdfPage.render(renderContext).promise.then(function(){
          ...
          if(magnetState.contains("invisible")){
            ...
            console.log(magnet.style.left,magnet.style.top,scale,magnet.getBoundingClientRect().x,magnet.getBoundingClientRect().y,pdfCanvas.getBoundingClientRect().y);
            tempX = magnet.getBoundingClientRect().x;
            tempY = magnet.getBoundingClientRect().y-pdfCanvas.getBoundingClientRect().y;
    ...
    socket.on(SYNC_EVENT,msg =>{//local=>sharedの切り替え
    ...
    socket.on(MAGNET_EVENT,msg =>{//マグネットの表示同期
      ...
      if(isZoomed){
        let scale = (pdfCanvas.width/(absolute.rx-absolute.lx) < pdfCanvas.height/(absolute.ry-absolute.ly))?
                    pdfCanvas.width/(absolute.rx-absolute.lx) : pdfCanvas.height/(absolute.ry-absolute.ly);
    ...
    socket.on(QUESTION_EVENT,msg =>{//マグネットの表示同期
      let magnetState = document.getElementById("magnet").classList;
    ...
    socket.on(PLAY_EVENT,msg =>{//Recorderからの受信
...
function drawSurface(){
  ...
  surfaceContext.fillRect(surface.lx,
                          surface.ly,
...
function onClick(e){
  ...
  if(gripping || magnetX - offsetX <= x && magnetY - offsetY <= y && x <= (magnetX - offsetX + magnet.width) && y <= (magnetY - offsetY +magnet.height)){
    ...
    if(gripping){
      ...
      var scale = (pdfCanvas.width/(absolute.rx-absolute.lx) < pdfCanvas.height/(absolute.rx-absolute.ly))?
              pdfCanvas.width/(absolute.rx-absolute.lx) : pdfCanvas.height/(absolute.rx-absolute.ly);

      if(isZoomed){
        magnetCoords[0]=parseInt((x-magnet.width/2)/scale + absolute.lx);
        magnetCoords[1]=parseInt((y-magnet.height/2)/scale + absolute.ly);
      }else{
  ...
  }else if(0 <= x && 0 <= y && x <= pdfCanvas.width && y <= pdfCanvas.height){
...
function onMove(e) {
  ...
  if(magnetX - offsetX <= x && magnetY - offsetY <= y && x <= (magnetX - offsetX + magnet.width) && y <= (magnetY - offsetY + magnet.height) && dragging){

    //magnet.getBoundingClientRect().x = x-(magnet.width/2);
    //magnet.getBoundingClientRect().y = y-(magnet.width/2);
    

   }else if(0 <= x && 0 <= y && x <= pdfCanvas.width && y <= pdfCanvas.height && dragging){
...
function doZoom(e){
  ...
  if(!isZoomed){//拡大
    ...
    var scale = (pdfCanvas.width/square.sw < pdfCanvas.height/square.sh)?
                  pdfCanvas.width/square.sw : pdfCanvas.height/square.sh;
...
let magnetState = document.getElementById("magnet").classList;
  ...
  if(isEnable){
    ...
    socket.emit(SYNC_EVENT,[absolute.lx,absolute.ly,absolute.rx,absolute.ry]);
    showFlame(shared);
  }
}
/******************************
...
function drawMagnet(magnetR){
  ...
  magnetContext.arc( magnetR/2, magnetR/2, magnetR/2-2, 0 * Math.PI / 180, 360 * Math.PI / 180, false ) ;
